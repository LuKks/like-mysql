...
(async () => {
  ...
  let result = await db.update(TABLE, { name: 'ef', code: 56 }, 'name = ?', 'ab');
  t.equal(db.sql, 'UPDATE ' + TABLE + ' SET `name` = ?, `code` = ? WHERE name = ?');
  t.equal(JSON.stringify(db.values), JSON.stringify(['ef', 56, 'ab']));
